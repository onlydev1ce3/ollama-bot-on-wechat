...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            ...
            #调知识库
            knowledge_switch_prefix = check_prefix(content, conf().get("knowledge_switch_prefix", [""]))
            if knowledge_switch_prefix:
                chat_request = {
                "messages": [
                    {
                        "role": "User",
                ...
                url = "http://127.0.0.1:6006/kb_example/"
                headers = {'Content-Type': 'application/json'}

                response = requests.post(url, headers=headers, data=json.dumps(chat_request))
                context['content']=json.dumps(response.json(), indent=4, ensure_ascii=False)

                if response.status_code == 200:
                    print("Response from /kb_example/:")
